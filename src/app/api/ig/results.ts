...
export async function GetResults(compId: string, resultsPage: string): Promise<ScrapedResultsType> {
  ...
  if (iframeElementHandle) {
    ...
    if (resultsTable) {
      ...
      const results: { igPosition: string, entrantName: string | null, score: string}[]  = await iframe.$$eval('tr.aggregate-row', rows => {
...
export async function ProcessResults(compId: string, resultsPage: string): Promise<ScrapedResultsCheckType> {
  const { sessionClaims } = auth();
  ...
  compEntrants.forEach(entrant => {
    const match = ggResults.scrapedResults.filter(result => (result.entrantName === entrant.entrant.systemName))
    if (match.length === 0) {
...
export async function GetEclectic(compId: string, resultsPage: string): Promise<ScrapedEclecticType> {
  const { sessionClaims } = auth();
  if (!sessionClaims?.metadata.adminPermission) throw new Error('NOT AUTHORISED')
  ...
  if (iframeElementHandle) {
    ...
    if (scorecardTable) {
      ...
      // console.log("format", format)
      returnData.compFormat = compFormat
      const results: { entrant: string, handicap: string, scores: EclecticScore[], entrantId: number | undefined, eclecticEntrantId: number | undefined }[]  = await iframe.$$eval('tr.net-line', rows => {
        // console.log("rows", rows)
        return rows.map(row => {
          const nameLink = row.querySelector("a.expand-tee-details")!
          const nameLinkText = nameLink.textContent?.replace(/[\u200B-\u200D\uFEFF]/g, '').replace("  ", " ")
          const name: string = nameLinkText?.substring(0,nameLinkText?.indexOf("(")).trim() ?? ""
          const handicap: string = nameLinkText?.substring(nameLinkText?.indexOf("(")+1,nameLinkText?.indexOf(")")) ?? ""
          ...
          for (let box = 0; box < scores.length; box++) {
            ...
            scoreBoxes.push({ hole: box+1, score: score?.textContent?.trim() ?? null })
          }
          
          
          return {
            entrant: name, handicap: handicap, scores: scoreBoxes as EclecticScore[], entrantId: entrants.filter(ent => ent.systemName === name)[0]?.id, eclecticEntrantId: eclecticEntrants.filter(ent => ent.systemName === name)[0]?.id
          }
        })  

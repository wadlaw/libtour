...
export const compRouter = createTRPCRouter({
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (comp?.open) {
        ...
        throw new TRPCError({
          ...
          message: "Comp is not open for entry!",
        });
      }
    }),
  ...
  withdraw: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (comp?.open) {
        ...
        throw new TRPCError({
          code: "PRECONDITION_FAILED",
          message: "All entries have been locked in!",
        });
      }
    }),
  ...
  getNonEntrants: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const enteredIds = await ctx.db.compEntrant.findMany({
        ...
        where: {
          OR: [{ compId: input.comp }, { comp: { shortName: input.comp } }],
        },
      });
  ...
  getTeamResultsForComp: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const stab = await ctx.db.comp.findFirst({
        where: {
          OR: [{ shortName: input.compId }, { igCompId: input.compId }],
        },
        select: {
          stableford: true,
        },
  ...
  processResults: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const addPoints = ctx.db.teamPoints.createMany({
        data: input.teamPoints,
      });

      const addPrizes = ctx.db.transaction.createMany({
  ...
  getAllWithPointsAndTeams: publicProcedure.query(({ ctx }) => {
    return ctx.db.comp.findMany({
      where: {
        completed: true,
        lib: true,

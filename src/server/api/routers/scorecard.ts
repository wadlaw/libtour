...
const CalculatePoints = (
  ...
  if (nr || typeof strokes === "undefined") {
    return 0;
  }
...
export const scorecardRouter = createTRPCRouter({
    ...
    .input(
      z
        .object({
          ...
          entrantId: z.number(),
          handicap: z.number(),
          stableford: z.boolean(),
          NR: z.boolean().optional().nullable().default(null),
          gross: z.number().optional().nullable().default(null),
          points: z.number().optional().default(0),
          net: z.number().optional().nullable().default(null),
          holes: z
            .object({
              holeNo: z.number(),
      ...
      await ctx.db.$transaction(
        input.map((sc) => {
          ...
          sc.holes.forEach((hole) => {
            (hole.points = CalculatePoints(
              hole.strokes,
              libbetsCard[hole.holeNo - 1]?.par ?? 4,
              libbetsCard[hole.holeNo - 1]?.strokeIndex ?? 19,
              sc.handicap,
              hole.NR,
            )),
              (hole.net = CalculateNet(
                hole.strokes,
                libbetsCard[hole.holeNo - 1]?.par ?? 4,
                libbetsCard[hole.holeNo - 1]?.strokeIndex ?? 19,
                sc.handicap,
          ...
          if (!sc.NR) {
            sc.gross = sc.holes.reduce(
              (acc, cur) => acc + (cur.strokes ?? 0),
              0,
            );
  ...
  addHoleDescription: adminProcedure
    .input(
      z.object({
  ...
  eagles: publicProcedure.query(({ ctx }) => {
  ...
  holesInOne: publicProcedure.query(({ ctx }) => {
    return ctx.db.hole.findMany({
      where: {
        NR: false,
        strokes: 1,
  ...
  doubleFigures: publicProcedure.query(({ ctx }) => {
    return ctx.db.hole.findMany({
      where: {
        NR: false,
        strokes: { gte: 10 },
  ...
  worstGrossRounds: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      return ctx.db.scorecard.findMany({
        ...
        orderBy: [{ strokesCountback: `desc` }],
        take: input.numberOfRounds,
      });
    }),

  LibEclecticScores: publicProcedure.query(async ({ ctx }) => {
  ...
  EclecticScores: publicProcedure.query(async ({ ctx }) => {
